...
GOOD_URL = f"{BASE_URL}/predict"
BAD_URL = f"{BASE_URL}/predictx"  # endpoint sengaja salah
...
def make_bad_payload():
    """Bikin payload yang sengaja bikin error di backend."""
    mode = random.choice(["empty", "missing_field", "invalid_type", "garbage_json"])
    ...
    if mode == "missing_field":
        ...
        # Hapus salah satu field required
        sample.pop("title", None)
        return sample

    if mode == "invalid_type":
        sample = random.choice(SAMPLES).copy()
        sample["title"] = 12345  # harusnya string
        sample["description"] = None
        return sample

    if mode == "garbage_json":
        # Nanti dikirim pakai data= dan bukan json=
        return "__THIS_IS_NOT_JSON__"

def main():
    i = 0
    while True:
        # 70% request error, 30% request normal
        is_error_request = random.random() < 0.7

        try:
            if is_error_request:
                error_type = random.choice(["bad_payload", "wrong_endpoint", "timeout"])
                
                if error_type == "bad_payload":
                    bad = make_bad_payload()
                    if isinstance(bad, str):
                        # Kirim raw string biar parsing JSON gagal
                        resp = requests.post(GOOD_URL, data=bad, timeout=5)
                        print(f"[{i}] bad_payload (raw) -> {resp.status_code} {resp.text[:200]}")
                    else:
                        resp = requests.post(GOOD_URL, json=bad, timeout=5)
                        print(f"[{i}] bad_payload (json) -> {resp.status_code} {resp.text[:200]}")

                elif error_type == "wrong_endpoint":
                    # Hit endpoint yang tidak ada → 404
                    payload = random.choice(SAMPLES)
                    resp = requests.post(BAD_URL, json=payload, timeout=5)
                    print(f"[{i}] wrong_endpoint -> {resp.status_code} {resp.text[:200]}")

                elif error_type == "timeout":
                    # Timeout sengaja kecil, supaya sering timeout
                    payload = random.choice(SAMPLES)
                    try:
                        resp = requests.post(GOOD_URL, json=payload, timeout=0.001)
                        print(f"[{i}] timeout_test -> {resp.status_code} {resp.text[:200]}")
                    except requests.exceptions.Timeout:
                        print(f"[{i}] timeout_test -> TIMEOUT (client)")
            else:
                # Request normal (harusnya 200)
                payload = random.choice(SAMPLES)
                resp = requests.post(GOOD_URL, json=payload, timeout=5)
                print(f"[{i}] OK -> {resp.status_code} {resp.json()}")

        except Exception as e:
            # Error di sisi client (gagal konek, dst.)
            print(f"[{i}] Client error:", repr(e))
